...
class MainViewModel(private val prefs: Prefs) : ViewModel() {
    ...
    fun isVersionIgnored(app: AppUpdate): Boolean {
        val ignoredVersions = prefs.ignoredVersions.get()
        return ignoredVersions.contains(app.id)
    };
    ...
    fun processIntent(
        ...
        when {
            ...
            intent.action?.contains(InstallAction).orFalse()  -> processInstallIntent(intent, launcher)
            else                                              -> {}
        }
    }
